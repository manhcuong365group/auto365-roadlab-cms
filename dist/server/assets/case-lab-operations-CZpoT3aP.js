...
import { _ as hashPassword, m as CaseLabApiError, n as createCaseDraft, p as normalizeCaseContentType, r as normalizeCaseDraft, t as assertCaseDraftWithinLimits, y as verifyPassword } from "./case-draft-DtbGA5cX.js";
...
async function listCaseRevisions(caseId, actor) {
	await requireScopedCase(caseId, actor);
	const rows = await getDb().select({
		revision: caseRevisions.revision,
		createdAt: caseRevisions.createdAt,
		createdBy: caseRevisions.createdBy
	...
	const authors = authorIds.length ? await getDb().select({
		id: users.id,
		displayName: users.displayName
	...
	return { items: rows.map((row) => ({
		...
		createdBy: nameById.get(row.createdBy) ?? row.createdBy
	})) };
}
async function getCaseRevisionContent(caseId, actor, revisionNumber) {
	const caseItem = await requireScopedCase(caseId, actor);
	const [revision] = await getDb().select().from(caseRevisions).where(and(eq(caseRevisions.caseId, caseId), eq(caseRevisions.revision, revisionNumber))).limit(1);
	if (!revision) throw new CaseLabApiError("NOT_FOUND", "Không tìm thấy revision.", 404);
	return {
...
var workflowActionLabels = {
	submit_review: "cần duyệt kỹ thuật",
	approve_technical: "đã qua duyệt kỹ thuật, cần duyệt SEO",
	approve_seo: "sẵn sàng xuất bản",
	request_changes: "cần chỉnh sửa lại",
	publish: "đã được xuất bản",
	archive: "đã bị huỷ/lưu trữ",
	restore: "đã được khôi phục về bản nháp"
};
async function notifyByRole(caseId, caseCode, branchRef, action, excludeUserId, createdAt) {
	const targetRoles = WORKFLOW_NOTIFY_ROLES[action];
	if (!targetRoles?.length) return;
	const roleRows = await getDb().select({
		userId: userRoles.userId,
		role: userRoles.role,
		branchRef: userRoles.branchRef
	}).from(userRoles);
	const recipientIds = [...new Set(roleRows.filter((row) => targetRoles.includes(canonicalRole(row.role)) && (row.branchRef === "*" || row.branchRef === branchRef) && row.userId !== excludeUserId).map((row) => row.userId))];
	if (!recipientIds.length) return;
	await getDb().insert(notifications).values(recipientIds.map((userId) => ({
		id: crypto.randomUUID(),
		userId,
		type: `case.${action}`,
		title: `${caseCode} ${workflowActionLabels[action]}`,
		body: "",
		caseId,
...
function requireBoss(actor) {
	if (!actor.roles.some((entry) => canonicalRole(entry.role) === "boss")) throw new CaseLabApiError("FORBIDDEN_ROLE", "Chỉ vai trò Sếp mới quản lý được tài khoản người dùng.", 403);
}
...
async function createUser(actor, input) {
	...
	if (existing) throw new CaseLabApiError("VALIDATION_ERROR", "Email này đã có tài khoản trong hệ thống.", 409);
	const createdAt = now();
	...
	for (const roleAssignment of input.roles) await getDb().insert(userRoles).values({
		userId,
		role: roleAssignment.role,
		branchRef: roleAssignment.branchRef,
		grantedAt: createdAt,
		grantedBy: actor.id
	});
	await getDb().insert(auditEvents).values({
		id: crypto.randomUUID(),
		actorId: actor.id,
		actorRole: actor.roles[0]?.role ?? "boss",
		action: "user.created",
		entityType: "user",
		entityId: userId,
...
async function updateUser(actor, userId, input) {
	...
	if (!target) throw new CaseLabApiError("NOT_FOUND", "Không tìm thấy tài khoản.", 404);
	const updatedAt = now();
	...
	if (input.password !== void 0) patch.passwordHash = await hashPassword(input.password);
	if (Object.keys(patch).length > 1) await getDb().update(users).set(patch).where(eq(users.id, userId));
	if (input.roles) {
	...
	await getDb().insert(auditEvents).values({
		...
		detailJson: JSON.stringify({
			displayName: input.displayName,
			status: input.status,
			passwordReset: input.password !== void 0,
			roles: input.roles
		}),
		createdAt: updatedAt
	});
...
async function changeMyPassword(actor, input) {
	const [user] = await getDb().select({
		id: users.id,
		passwordHash: users.passwordHash
	}).from(users).where(eq(users.id, actor.id)).limit(1);
	if (!user || !user.passwordHash || !await verifyPassword(input.currentPassword, user.passwordHash)) throw new CaseLabApiError("VALIDATION_ERROR", "Mật khẩu hiện tại không đúng.", 400);
	const updatedAt = now();
...
//#endregion
export { assignCaseReviewer, changeMyPassword, createCase, createCaseFeedback, createUser, getCaseDraft, getCaseRevisionContent, getDashboard, getMyProfile, getOperationsReport, listCaseAssignments, listCaseAudit, listCaseFeedback, listCaseRevisions, listMyNotifications, listUsers, markNotificationRead, resolveCaseFeedback, saveCaseDraft, transitionCaseStatus, updateMyProfile, updateUser };
