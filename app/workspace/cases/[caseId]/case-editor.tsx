...
import type { RoadLabDraft } from "../../../../lib/road-lab-draft";
...
export default function CaseEditor({ caseId, mode }: { caseId: string; mode: "editor" | "review" }) {
  ...
  if (loading) return <main className="workspace-page"><section className="workspace-shell"><p className="workspace-loading">Đang tải case…</p></section></main>;
  if (error && !data) return <main className="workspace-page"><section className="workspace-shell"><Link className="workspace-back" href="/workspace/cases">← Case / bài viết</Link><div className="workspace-alert" role="alert">{error === "SESSION_REQUIRED" ? <><b>Phiên đăng nhập chưa sẵn sàng</b><Link className="workspace-alert__link" href={`/login?return_to=/workspace/cases/${encodeURIComponent(caseId)}`}>Đăng nhập Case Lab →</Link></> : error}</div></section></main>;
  if (!data || !form) return null;

  return <main className="workspace-page"><section className="workspace-shell workspace-case-page">
